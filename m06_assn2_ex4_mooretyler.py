...
# author: moore, t
# date: 2021-09-26
# program: m06_assn2_ex4_mooretyler.py
# purpose: final project with message label
...
class OrderForm(Frame):
	def __init__(self, root):
		...
		lblError = ttk.Label(MainFrame, text='', foreground='red', font=('',14), textvariable=root.ErrorMessage)
		lblError.grid(column=0, row=11, columnspan=2, sticky=W)
	
		# defines the final buttons and placement
		btnOrder = ttk.Button(MainFrame, text='Order').grid(column=1, row=12)
		btnClear = ttk.Button(MainFrame, text='Clear', command=self.clear).grid(column=2, row=12, columnspan=1, sticky=W)
